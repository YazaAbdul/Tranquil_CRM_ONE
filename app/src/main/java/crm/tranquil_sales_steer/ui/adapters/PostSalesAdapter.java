...
public class PostSalesAdapter extends RecyclerView.Adapter<PostSalesAdapter.ShortcutsVH> {

    ArrayList<PostSalesResponse> shortcutsResponses = new ArrayList<>();
    Activity activity;
    PostSalesClickListener shortcutsClickListener;

    public PostSalesAdapter(ArrayList<PostSalesResponse> shortcutsResponses, Activity activity, PostSalesClickListener shortcutsClickListener) {
    ...
    public ShortcutsVH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ShortcutsVH(LayoutInflater.from(parent.getContext()).inflate(R.layout.post_sales_list_item,parent,false));
    }
    @Override
    public void onBindViewHolder(@NonNull ShortcutsVH holder, int position) {

        PostSalesResponse item = shortcutsResponses.get(position);

        Picasso.with(activity).load(item.getImage()).into(holder.shortcutsBackgroundIVID);
    ...
    /*    if (shortcutsResponses.get(position).getColor_start() !=null && shortcutsResponses.get(position).getColor_end()!= null){
        ...
        holder.shortcutsRLID.setOnClickListener(v -> shortcutsClickListener.onPostSalesItemClicked(item,position,v,holder));
    ...
    public interface PostSalesClickListener {
        void onPostSalesItemClicked(PostSalesResponse shortcutsResponse, int position, View v, ShortcutsVH holder);
    }
}
